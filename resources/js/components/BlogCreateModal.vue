...
export default {
    ...
    methods: {
        ...
        async submit() {
            this.loading = true
            try {
                const res = await axios.post('/blogs', this.form)
                console.log(res.data) // ← lihat dulu di console key-nya apa

                const blog = res.data.blog || res.data.Blog || res.data

                this.close() // ← close dulu
                this.$emit('created', blog) // ← baru emit
